...
def main():
    ...
    for line in sys.stdin:
        ...
        try:
            ...
            voltage = (from_twos_complement(cs[3]) + st.adc_offset_trim_c3) \
                          * st.scale_c3 * st.adc_gain_trim_c3
            ...
            if st.current_axis_per_division <= 0.1:
                current = (from_twos_complement(cs[1]) + st.adc_offset_trim_c1) \
                          * st.scale_c1 * st.adc_gain_trim_c1
            else:
                ...
                current = (from_twos_complement(cs[2]) + st.adc_offset_trim_c2) \
                              * st.scale_c2 * st.adc_gain_trim_c2
            power = voltage * current
            leakage_current = (from_twos_complement(cs[0]) + st.adc_offset_trim_c0) \
                              * st.scale_c0 * st.adc_gain_trim_c0
        except ValueError:
