...
def adc_read_handler(dr_adc):
    global read_buffer, current_ring_buffer, in_ptr
    spi_adc.readinto(read_buffer)
    current_ring_buffer[in_ptr*8 : in_ptr*8+8] = read_buffer
...
def main():
    global ring_buffer1, ringbuffer2, in_ptr, out_ptr
    ...
    for i in range(len(ring_buffer1)):
        ring_buffer1[i] = i % 256
        ring_buffer2[i] = (i+50) % 256
    ...
    while True:
        ...
        print(binascii.hexlify(current_ring_buffer))
        if current_ring_buffer == ring_buffer1:
            current_ring_buffer = ring_buffer2
        else:
            current_ring_buffer = ring_buffer1
 

# Buffer memory visible to both threads
# two contiguous mutable bytearrays for samples
ring_buffer1 = bytearray(BUFFER_SIZE * 8)
ring_buffer2 = bytearray(BUFFER_SIZE * 8)
current_ring_buffer = ring_buffer1
