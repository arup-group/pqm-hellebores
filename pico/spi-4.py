...
def setup_adc(spi, cs, reset):
    ...
    print("Setting gain register 0x0b to 0x00, 0x0a, 0x00.")
    set_and_verify_adc_register(spi, cs, 0x0b, bytes([0x00,0x0a,0x00]))
    time.sleep(1)
    ...
    print("Setting configuration register CONFIG0 at 0x0d to 0x24, 0x20, 0x50.")
    ...
    set_and_verify_adc_register(spi, cs, 0x0d, bytes([0x24,0x20,0x50]))
    time.sleep(1)
...
def initialise():
    ...
    # initialise a buffer for reading
    acq = bytearray(8)
    spi_adc.readinto(acq)   # first reading


def display_update(ch):
